...
const ru: DiagnosticsCopy = {
  title: "Проверить маршрутизацию Codex", body: "Настроенный провайдер, папка Codex и наблюдаемые запросы списка моделей.",
  checking: "Проверка маршрутизации…", refresh: "Проверить снова", failed: "Не удалось прочитать диагностику маршрутизации. Повторите попытку или запустите диагностику среды.",
  scope: "Значения взяты из конфигурации Codex, используемой NEKODEX. Работающий клиент Codex может использовать другую папку, профиль или параметры командной строки. Проверка не меняет настройки.",
  home: "Папка Codex", file: "Основной файл настроек", profileFile: "Файл настроек профиля", profile: "Настроенный профиль", provider: "Настроенный провайдер", source: "Источник настройки провайдера", config: "Конфигурация", integration: "Интеграция", route: "Настроенный маршрут", catalog: "Запросы списка моделей", last: "Последний успешный запрос",
  none: "По умолчанию (профиль не выбран)", unknown: "Недоступно", installed: "Установлена", absent: "Не установлена", active: "Активна, соответствует установленному маршруту", inactive: "Неактивна", mismatch: "Активна, настройки отличаются", unverified: "Не удалось проверить маршрут",
  loaded: "Прочитана", missing: "Файл отсутствует", invalid: "Некорректная конфигурация", unreadable: "Не удалось прочитать файл",
  default: "По умолчанию", root: "Основная конфигурация", profileSource: "Выбранный профиль", observed: "Наблюдались", waiting: "Ожидание первого успешного запроса", unavailable: "Сведения недоступны",
  observedBody: "Текущий процесс моста получил успешный запрос списка моделей. Выбор модели и завершение задач проверяются отдельно.",
  waitingBody: "После установки перезапустите Codex и откройте новую задачу. Если моделей нет, проверьте, что Codex использует указанную папку и профиль. Одна переустановка не объясняет отсутствие запроса списка моделей.",
  unavailableBody: "Не удалось подтвердить сведения о запросах к текущему процессу. Запустите диагностику среды и повторите проверку.",
  custom: "Настроен пользовательский провайдер, который может обходить этот мост. Перед изменением конфигурации уточните, какой провайдер используется нужной задачей Codex.",
  override: "Настроена локальная подмена списка моделей, которая может заменять список моста. Проверьте этот параметр в указанном файле.",
  routeMismatch: "Настроенный маршрут отличается от установленного моста. Перед повторным подключением проверьте профиль и состояние интеграции в диагностике.",
  drift: "Установленная интеграция отличается от сохранённой конфигурации. Перед повторным подключением проверьте расхождения в диагностике.",
  configProblem: "Не удалось надёжно прочитать настройки Codex. Проверьте указанный файл: сведения о провайдере и маршруте могут быть неполными.",
  profileProblem: "Не удалось определить настроенный профиль. Проверьте выбранный профиль и его файл настроек.",
  providerProblem: "Не удалось определить настроенного провайдера. Проверьте его параметры в файле конфигурации.",
  integrationProblem: "Не удалось прочитать состояние интеграции. Запустите диагностику для новой проверки.",
