const { UNKNOWN_ACCOUNT_ID, OUTCOMES, COUNTERS, FAILURE_CODES, NATIVE_OUTCOMES, NATIVE_COUNTERS, dayOf, classificationKey, rowKey, emptyGroup, nativeGroupKey, nativeRowKey, emptyNativeGroup, normalizedFailureCode } = require('./usage-schema.cjs');
const { projectWebUsage, projectNativeUsage } = require('./usage-report.cjs');
...
class UsageStore {
  ...
  snapshotNative(days) {
    ...
    return projectNativeUsage(this.state, days, this.clock(), { error: this.error, recovered: this.recovered, backupAvailable: this.backupAvailable });
  }
  ...
  snapshot(query = 7, accountMetadata = []) {
    ...
    return projectWebUsage(this.state, { days, accountId }, accountMetadata, this.clock(), { error: this.error, recovered: this.recovered, backupAvailable: this.backupAvailable });
  }
}
