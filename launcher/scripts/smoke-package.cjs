...
try {
  ...
  if (process.platform === "darwin") {
    ...
    macAppBundle = path.join(stage, "NEKODEX.app");
    executable = path.join(macAppBundle, "Contents", "MacOS", "NEKODEX");
    command = executable;
  ...
  try {
    if (macAppBundle) {
      ...
      // Unregister only this task-owned bundle; leave unrelated app registrations alone.
    }
  } finally {
    try {
      ...
      process.stderr.write(`Temporary package smoke files remain at ${scratch}: ${error.code}\n`);
    }
  }
}
