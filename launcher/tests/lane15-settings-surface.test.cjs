...
test('guarded actions reject same-tick duplicate calls and cancelled removal preserves diagnostics', async () => {
  let finish, calls = 0;
  const h = harness({ uninstallIntegration: () => { calls++; return new Promise(resolve => { finish = resolve; }); } });
  const remove = () => h.find(node => node.type === 'button' && JSON.stringify(node.props.children).includes('uninstall'));
  const initial = h.find(node => node.type === 'RouteDiagnostics').props.key;
  assert.ok(remove()); remove().props.onClick(); remove().props.onClick();
  assert.equal(calls, 1);
  finish({ cancelled: true }); await tick(); h.render();
  assert.equal(h.find(node => node.type === 'RouteDiagnostics').props.key, initial);
  remove().props.onClick(); assert.equal(calls, 2);
  finish({ cancelled: false, state: { browserInteractionMode: 'automatic' } }); await tick(); h.render();
  assert.notEqual(h.find(node => node.type === 'RouteDiagnostics').props.key, initial);
});
