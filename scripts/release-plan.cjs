...
function releasePlan({ event, refType, tag, version, scope = "all", windowsDelivery = "signed" }) {
  ...
  if (event !== "workflow_dispatch") { scope = "all"; windowsDelivery = "signed"; }
  if (!["signed", "preview"].includes(windowsDelivery)) throw new Error("Unsupported Windows delivery");
  if (windowsDelivery === "preview" && (!version.includes("-") || scope !== "all")) throw new Error("Windows preview requires an all-platform prerelease");
  ...
  if (scope === "all") include.push(
    ...
    ...(windowsDelivery === "signed" ? [{ runner: "windows-latest", runtime_asset: "codex-chatgpt-web-windows-amd64.zip", notices: false }] : []),
  );
  return { include };
}
if (require.main === module) {
  const matrix = releasePlan({ event: process.env.GITHUB_EVENT_NAME, refType: process.env.GITHUB_REF_TYPE,
    tag: process.env.GITHUB_REF_NAME, version: manifest.version, scope: process.env.RELEASE_PLATFORM_SCOPE || "all", windowsDelivery: process.env.RELEASE_WINDOWS_DELIVERY || "signed" });
