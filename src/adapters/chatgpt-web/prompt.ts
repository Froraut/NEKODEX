import { formatChatGptWebMultipartCommit, formatChatGptWebMultipartStage, isChatGptWebMultipartPartCount, type ChatGptWebMultipartPartCount, type ChatGptWebMultipartParts, type ChatGptWebMultipartPrompt } from "./prompt-multipart-contract";
...
export interface CompiledChatGptWebPrompt {
  ...
  /** Expendable history items omitted to fit inline compaction; absent on normal turns. */
...
export function chatGptReadOnlyContextWarning(
  ...
  const browserOnlyGuidance = !capabilities.localToolsEnabled
    ? "\n>\n> **Action:** Open `MCP` in `NEKODEX` and connect the `Full` harness to give the selected ChatGPT Web model access to local tools."
    : "";
  if (hasLocalEvidence) {
...
export function compileChatGptWebPrompt(
  ...
  const build = (sourceMessages: readonly CodexMessage[]): CompiledChatGptWebPrompt => {
    ...
    if (multipartEnabled) {
      ...
      const budgets = multipart.parts.map((payload, index) => {
        ...
        const effort = final ? mode.effort : capabilities.proAvailable && options?.allowProStaging !== false ? "max" : "medium";
  ...
  // one even when it precedes a large tool transcript, so fallback compaction cannot silently
  // reset cumulative progress to the most recent tool results.
