...
test("upstream #416: a four-position hidden slider admits Extra High while Pro stays unavailable", async () => {
  // Reported by aktsmsm: https://github.com/miuuyy/codex-chatgpt-web/issues/416.
  const fixture = reasoningPicker({ max: "3" });
  const capabilities = await detectChatGptAccountCapabilities(fixture.page as never);
  expect(capabilities).toEqual({ solAvailable: true, extraHighAvailable: true, proAvailable: false });
  const select = (ChatGptBrowserWorker.prototype as unknown as {
    selectModelAndEffort(...args: unknown[]): Promise<unknown>;
  }).selectModelAndEffort;
  await select.call({ activeComposer: async () => fixture.composer }, fixture.page, "gpt-5.6-sol", "xhigh", {
  ...
  await expect(select.call({ activeComposer: async () => fixture.composer }, fixture.page, "gpt-5.6-sol", "max", {
    ...capabilities, localToolsEnabled: false,
  })).rejects.toThrow("Pro effort is not available");
});

test("unsupported slider cardinality cannot silently admit an unknown reasoning mode", async () => {
  await expect(detectChatGptAccountCapabilities(reasoningPicker({ max: "1" }).page as never))
    .rejects.toThrow("unsupported reasoning range");
  await expect(detectChatGptAccountCapabilities(reasoningPicker({ max: "5" }).page as never))
    .rejects.toThrow("model controls are unavailable");
});
