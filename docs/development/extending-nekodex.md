...
| Change ChatGPT DOM selectors or extraction | `browser-submission-dom.ts`, `browser-response-dom.ts`, `browser-dom-revision.ts` | Submission history and bound-response caches retain their caller-owned identity. Browser send/rebind decisions and each stage's budget remain in `browser-worker.ts`; submission-evidence and turn-identity rules are in the pure `browser-submission-policy.ts`. |
| Change stage budgets or sleep refund | `browser-stage-budget.ts` for mechanics; `browserStageTimeouts` in `browser-worker.ts` for per-stage values | Refund only detected process suspension, never awake time. A stage that times out must still abort and, where requested, await its action's settlement. |
| Change blocking dialog, alert or tool-approval handling | `browser-page-guards.ts` | Guards classify page state without owning it. Model selection receives them by injection; send/rebind decisions stay in `browser-worker.ts`. |
...
| Change multipart transport or capacity | `prompt-multipart-contract.ts`, `prompt-context-envelope.ts`, `browser-input-policy.ts` | Prompt compilation chooses content; `prompt-context-envelope.ts` scrubs retired handles and partitions whole records; stage/commit formatting and account/input capacity are separately testable contracts. |
...
| Change allowed ChatGPT/auth navigation or log-safe navigation fields | `launcher/electron/browser-navigation-policy.cjs` | Pure predicates plus a guard over caller-supplied contents; Codex sign-in windows share the auth allowlist. Views, webRequest filters and state stay in `browser-host.cjs`, which re-exports the existing names. |
| Change the launcher's in-page session probe | `launcher/electron/browser-auth-probe.cjs` | The page script returns observations only; `browser-host.cjs` owns probe sequencing, stale-probe rejection, identity epochs and published state. |
...
| Change update preparation | `launcher/electron/update-staging.cjs` | Before staging returns it owns cleanup; the controller owns handoff after return. Hashing is abortable; uncertain extractor exit preserves staging. The worker files that staging copies (`update-worker`, `update-validation`, `update-recovery`, `update-launcher`) must stay self-contained and never require controller-side helpers such as `update-abort.cjs`. Staging prunes cached packages other than the selected one. |
...
| Change native menu, tray or dialog copy | `launcher/electron/native-copy.cjs` | Static tables with English fallback only; language selection and launcher-state reads stay in `main.cjs`. |
...
| Extend an application screen | `launcher/src/Onboarding.tsx`, `ActivitySurface.tsx`, `SetupSurface.tsx`, `McpSurface.tsx`, `SettingsSurface.tsx`, `BrowserSurface.tsx` | `App.tsx` owns navigation, shared snapshots and native browser placement; `AppShell.tsx` holds prop-driven shell chrome and `useUpdateControls.ts` the update actions. `BrowserSurface.tsx` owns browser controls and the account-pool selection; `BrowserWorkspaceManager.tsx` uses that same selection for separate windows and native window tabs; `ManualTurnGuide.tsx` owns prompt presentation and its countdown. |
