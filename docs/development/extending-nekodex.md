...
| Change update preparation | `launcher/electron/update-staging.cjs` | Before staging returns it owns cleanup; the controller owns handoff after return. Hashing is abortable; uncertain extractor exit preserves staging. The worker files that staging copies (`update-worker`, `update-validation`, `update-recovery`, `update-launcher`) must stay self-contained and never require controller-side helpers such as `update-abort.cjs`. |
