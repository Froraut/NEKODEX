...
messages; at twice that threshold it uses six messages. Message-size limits can require six parts
earlier, even below that token threshold. The final context part also commits the
...
six parts. The final part retains the selected execution effort. Larger transactions may increase the probability of
